...
public class LessCompilerTest {
	 ...
	 * 
	 */
	private static final String CONTEXT_FIELD = "_context";
	...
	@Mock
	private URL envJsFile;

	@Mock
	private URLConnection envJsURLConnection;

	@Mock
	private InputStream envJsInputStream;

	@Mock
	private InputStreamReader envJsInputStreamReader;

	@Mock
	private URL lessJsFile;

	@Mock
	private URLConnection lessJsURLConnection;

	@Mock
	private InputStream lessJsInputStream;

	@Mock
	private InputStreamReader lessJsInputStreamReader;
	...
	public void setUp() throws Exception {
		...
		when(Boolean.valueOf(log.isDebugEnabled())).thenReturn(Boolean.FALSE);

		FieldUtils.writeField(lessCompiler, "logger", log, true);
	}

	@Test
	public void testNewLessCompiler() throws Exception {
		assertEquals(
				LessCompiler.class.getClassLoader().getResource(
						"META-INF/env.rhino.js"),
				FieldUtils.readField(lessCompiler, "_envJs", true));
		assertEquals(
				LessCompiler.class.getClassLoader().getResource(
						"META-INF/less.js"),
				FieldUtils.readField(lessCompiler, "_lessJs", true));
		assertEquals(Collections.EMPTY_LIST,
				FieldUtils.readField(lessCompiler, "_customJs", true));
	}
	...
	public void testInit() throws Exception {
		...
		whenNew(InputStreamReader.class).withArguments(envJsInputStream)
				.thenReturn(envJsInputStreamReader);

		when(lessJsFile.openConnection()).thenReturn(lessJsURLConnection);
		when(lessJsURLConnection.getInputStream())
				.thenReturn(lessJsInputStream);
		whenNew(InputStreamReader.class).withArguments(lessJsInputStream)
				.thenReturn(lessJsInputStreamReader);
		...
		verify(cx).evaluateReader(scope, envJsInputStreamReader,
				"env.rhino.js", 1, null);
		...
		verify(cx).evaluateReader(scope, lessJsInputStreamReader, "less.js", 1,
				null);
	}

	@Test(expected = IllegalStateException.class)
	public void testInitThrowsIllegalStateExceptionWhenNotAbleToInitilize()
			throws Exception {
		mockStatic(Context.class);
		when(Context.enter()).thenReturn(cx);
	...
	public void testCompileFileToString() throws Exception {
		...
		whenNew(LessSource.class).withArguments(inputFile).thenReturn(
				lessSource);
	...
	@Test
	public void testCompileFileToFileWithForceFalseAndOutputNotExists()
			throws Exception {
		FieldUtils.writeField(lessCompiler, CONTEXT_FIELD, cx, true);
		FieldUtils.writeField(lessCompiler, "_scope", scope, true);

		when(Boolean.valueOf(outputFile.exists())).thenReturn(Boolean.FALSE);
	...
	@Test
	public void testCompileFileToFileWithForceFalseAndOutputExistsAndLessSourceModified()
			throws Exception {
		FieldUtils.writeField(lessCompiler, CONTEXT_FIELD, cx, true);
		FieldUtils.writeField(lessCompiler, "_scope", scope, true);
		...
		when(Boolean.valueOf(outputFile.exists())).thenReturn(Boolean.TRUE);
		when(Long.valueOf(outputFile.lastModified())).thenReturn(
				Long.valueOf(1));

		when(Long.valueOf(lessSource.getLastModifiedIncludingImports()))
	...
	@Test
	public void testCompileFileToFileWithForceFalseAndOutputExistsAndLessSourceNotModified()
			throws Exception {
		FieldUtils.writeField(lessCompiler, CONTEXT_FIELD, cx, true);
		FieldUtils.writeField(lessCompiler, "_scope", scope, true);
	...
	@Test
	public void testCompileLessSourceToFileWithForceFalseAndOutputNotExists()
			throws Exception {
		FieldUtils.writeField(lessCompiler, CONTEXT_FIELD, cx, true);
		FieldUtils.writeField(lessCompiler, "_scope", scope, true);
	...
	@Test
	public void testCompileLessSourceToFileWithForceFalseAndOutputExistsAndLessSourceModified()
			throws Exception {
		FieldUtils.writeField(lessCompiler, CONTEXT_FIELD, cx, true);
		FieldUtils.writeField(lessCompiler, "_scope", scope, true);
	...
	@Test
	public void testCompileLessSourceToFileWithForceFalseAndOutputExistsAndLessSourceNotModified()
			throws Exception {
		FieldUtils.writeField(lessCompiler, CONTEXT_FIELD, cx, true);
		FieldUtils.writeField(lessCompiler, "_scope", scope, true);
	...
	public void testCompileThrowsLessExceptionWhenCompilationFails()
			throws Exception {
		FieldUtils.writeField(lessCompiler, CONTEXT_FIELD, cx, true);
		FieldUtils.writeField(lessCompiler, "_scope", scope, true);

		when(cx.evaluateString(scope, COMPILE_STRING, "compile.js", 1, null))
				.thenThrow(new JavaScriptException(null, null, 0));
	...
	public void testCompress() throws Exception {
		...
		verify(cx)
				.evaluateString(
						scope,
						"var result; var parser = new(less.Parser); parser.parse(input, function (e, tree) { if (e instanceof Object) { throw e } result = tree.toCSS({compress: true}) });",
						"compile.js", 1, null);
